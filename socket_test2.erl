...
main([PortStr]) ->
    ...
    try
        ...
        io:format(
            "--- Test: System Default Socket Behavior , Port: ~w ---~n", [Port]
        ),
        ...
        error:badarg ->
            io:format(
                "Error: Invalid port number provided.~nUsage: erl -noshell -s socket_test main <port>~n",
                []
            ),
            halt(1)
    end;
...
run_default_test(Port) ->
    try
        ...
        Result =
            try
                {ok, ListenIPv4} = gen_tcp:listen(Port, [
                    {ip, {0, 0, 0, 0}},
                    {reuseaddr, true},
                    binary,
                    {active, false}
                ...
                io:format(
                    "Result: Success! Your system allows separate IPv4 and IPv6 "
                    "binds by default.~n"
                ),
                ...
                error:{badmatch,{error,eaddrinuse}} ->
                    io:format(
                        "Error: Could not open IPv4 socket, address is already in use.~n"
                    ),
                    io:format(
                        "Result: Conflict! Your system's default is likely a dual-stack "
        ...
        Type:Reason ->
            io:format("Error opening initial IPv6 socket: ~p:~p~n", [
                Type, Reason
            ]),
...
run_v6only_test(Port) ->
    try
        ...
        io:format("OK: IPv6 socket with 'ipv6_v6only' opened on [::]:~p.~n", [
            Port
        ]),
        Result =
            try
                ...
                io:format("OK: IPv4 socket also opened on 0.0.0.0:~p.~n", [Port]),
                io:format(
                    "Result: Success! ipv6_v6only=true allows separate IPv4 and IPv6 sockets.~n"
                ),
                gen_tcp:close(ListenIPv4),
                separate_sockets
            catch
                error:eaddrinuse ->
                    ...
                    io:format(
                        "Result: Unexpected! Even with ipv6_v6only=true, there's still a conflict.~n"
                    ),
                    unexpected_conflict
            end,
        gen_tcp:close(ListenIPv6),
